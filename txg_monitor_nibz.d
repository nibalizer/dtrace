...
/self->start && (this->ms = (timestamp - self->start) / 1000000) > MIN_MS/
{
	...
	printf("%s.%s", "nibz.production.vault.txg_montior.delays", stringof(self->spa->spa_name));
	printa(" %@d ", @delays);
	printf("%d\n", walltimestamp/1000000000);
	printf("%s.%s", "nibz.production.vault.txg_montior.throttles", stringof(self->spa->spa_name));
	printa(" %@d ", @throttles);
	printf("%d\n", walltimestamp/1000000000);
